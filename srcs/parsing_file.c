	...
	while (current)
	{
		...
		if (!count_nb_element(line, &cam, &ambient, &light))
		{
			if (ft_strncmp(line, "sp", 2) != 0 && ft_strncmp(line, "pl", 2) != 0
				...
				return (error("Invalid element id."), 0);
			(*obj_count)++;
		}
		current = current->next;
	}
	if (cam != 1 || ambient != 1 || light != 1 || *obj_count == 0)
		return (error("Must be 1 cam,ambient,light,shape."), 0);
	return (1);
}
...
int	parse_scene(char *file, t_scene *scene)
{
	...
	if (!scene->objects)
		return (ft_lstclear(&scene->lines, free), perror("Malloc"), 0);
	current = scene->lines;
	while (current)
	{
		line = current->content;
		if (!parse_element_line(line, scene, &element_count))
			return (ft_lstclear(&scene->lines, free), free(scene->objects), 0);
		current = current->next;
	}
	ft_lstclear(&scene->lines, free);
	prep_initial_comps(&scene->camera, scene->objects, scene->obj_count);
	return (1);
}
