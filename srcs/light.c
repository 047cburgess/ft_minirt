...
// diffuse: if dot product == 1 -> max light
// dot compares angle between 2 directions (normal and light dir)
// If hits another object before light, is in shadow (ambient)
void	apply_lighting(t_scene *scene, t_xs *hit, int *final_color)
{
	...
	else
	{
		...
		{
			scalar += specular(hit->world_position,
					hit->world_normal, light_direction, scene);
		}
		if (scalar > 1.0)
			scalar = 1;
	}
...
// Model phong
double	specular(t_tuple hit, t_tuple normal, t_tuple light_dir, t_scene *scene)
{
	t_tuple		cam_dir;
	t_tuple		reflect_dir;
	...
	light = scene->light;
	cam_dir = normalize(subtract(scene->camera.position, hit));
	reflect_dir = subtract(scale(normal, 2.0
				* dot(normal, light_dir)), light_dir);
	reflect_dir = normalize(reflect_dir);
	dot_reflect_cam = dot(reflect_dir, cam_dir);
	if (dot_reflect_cam < 0)
		dot_reflect_cam = 0;
	specular = light.coeff * light.ratio * pow(dot_reflect_cam, light.shine);
	return (specular);
}
